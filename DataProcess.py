...
def get_query_templates(language: str = "eng"):
    """
    根據語言回傳對應的 Query 模板。
    language:
        - "eng"：英文版（原始設定）
    ...
    if language == "zho":
        # 你可以依喜好微調字串，但長度/結構盡量跟原本類似
        forward_aspect_query_template = ["[CLS]", "有哪些", "方面", "？", "[SEP]"]

        # [CLS] 在 這個 方面 下 有哪些 評價 ？ [SEP]
        ...
        # [CLS] 這個 評價 是在說 哪一個 方面 ？ [SEP]
        backward_aspect_query_template = [
            "[CLS]", "這個", "評價", "是", "在", "說", "哪一個", "方面", "？", "[SEP]"
        ]

        # [CLS] 這個 方面 和 評價 對應 到 哪一個 類別 ？ [SEP]
        category_query_template = [
            "[CLS]", "這個", "方面", "和", "評價", "對應", "到", "哪一個", "類別", "？", "[SEP]"
        ]

        # [CLS] 這個 方面 和 評價 在 愉悅度 上 是 多少 ？ [SEP]
        valence_query_template = [
            "[CLS]", "這個", "方面", "和", "評價", "在", "愉悅度", "上", "是", "多少", "？", "[SEP]"
        ]

        # [CLS] 這個 方面 和 評價 在 喚醒度 上 是 多少 ？ [SEP]
        arousal_query_template = [
            "[CLS]", "這個", "方面", "和", "評價", "在", "喚醒度", "上", "是", "多少", "？", "[SEP]"
        ]

    else:
        # 保留原本的英文模板，方便你之後跑英文資料
        forward_aspect_query_template = ["[CLS]", "what", "aspects", "?", "[SEP]"]
        forward_opinion_query_template = [
        ...
        category_query_template = [
            "[CLS]", "what", "category", "given", "the", "aspect", "and", "the", "opinion", "?", "[SEP]"
        ]
        valence_query_template = [
            "[CLS]", "what", "valence", "given", "the", "aspect", "and", "the", "opinion", "?", "[SEP]"
        ]
        arousal_query_template = [
            "[CLS]", "what", "arousal", "given", "the", "aspect", "and", "the", "opinion", "?", "[SEP]"
        ]
...
def make_QA(args, line, word_list, aspect_list, opinion_list, category_list, valence_list, arousal_list):
    ...
    (
        ...
        valence_query_template,
        arousal_query_template,
    ) = get_query_templates(getattr(args, "language", "eng"))
...
def inference_data_process(args, text):
    ...
    # 🔍 DEBUG B-0
    print("[DEBUG B0] inference_data_process: input len(text) =", len(text))
    ...
    for idx, line in enumerate(text[:]):
        ...
        if idx < 3:
            print(f"[DEBUG B0] sample {idx}: ID={line[0]}, text_snippet={line[1][:30]}...")

    print("[DEBUG B0] number of inference samples:", len(ID_list))
...
def dataset_inference_process(args, datasets, category_mapping, tokenizer):
    ...
    # 🔍 DEBUG B-1
    print("[DEBUG B1] dataset_inference_process: raw dataset len =", len(datasets))
    ...
    # 🔍 DEBUG B-2
    print("[DEBUG B2] inference_data_process result: len(ID_list) =", len(ID_list),
          "len(QA_list) =", len(QA_list), "max_len =", max_len)
    if len(ID_list) > 0:
        print("[DEBUG B2] first 3 IDs:", ID_list[:3])
